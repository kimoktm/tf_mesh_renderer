...
REGISTER_OP("RasterizeTrianglesGradGrad")
    .Input("vertices: float32")
    .Input("triangles: int32")
    .Input("barycentric_coordinates: float32")
    .Input("triangle_ids: int32")
    .Input("df_dvertices: float32")
    .Attr("image_width: int")
    .Attr("image_height: int")
    .Output("df_df_dbarycentric_coordinates: float32");

class RasterizeTrianglesGradGradOp : public OpKernel {
 public:
  explicit RasterizeTrianglesGradGradOp(OpKernelConstruction* context)
      : OpKernel(context) {
  ...
  ~RasterizeTrianglesGradGradOp() override {}
  ...
  void Compute(OpKernelContext* context) override {
    ...
    OP_REQUIRES(
        ...
        InvalidArgument(
            "RasterizeTrianglesGradGrad expects vertices to have shape (-1, 3)."));
    ...
    OP_REQUIRES(
        ...
        InvalidArgument(
            "RasterizeTrianglesGradGrad expects triangles to be a matrix."));
    ...
    OP_REQUIRES(context,
                ...
                InvalidArgument(
                    "RasterizeTrianglesGradGrad expects barycentric_coordinates to "
    ...
    OP_REQUIRES(
        ...
        InvalidArgument(
            "RasterizeTrianglesGradGrad expected triangle_ids to have shape "
    ...
    const Tensor& df_dvertices_tensor = context->input(4);
    OP_REQUIRES(
        context,
        TensorShape({vertex_count, 3}) ==
            df_dvertices_tensor.shape(),
        InvalidArgument(
            "RasterizeTrianglesGradGrad expects df_dvertices "
            "to have shape {vertex_count, 3}"));
    auto df_dvertices_flat =
        df_dvertices_tensor.flat<float>();
    const float* df_dvertices =
        df_dvertices_flat.data();


    Tensor* df_df_dbarycentric_coordinates_tensor = nullptr;
    OP_REQUIRES_OK(context,
                   context->allocate_output(0, TensorShape({image_height_, image_width_, 3}),
                                            &df_df_dbarycentric_coordinates_tensor));
    auto df_df_dbarycentric_coordinates_flat = df_df_dbarycentric_coordinates_tensor->flat<float>();
    float* df_df_dbarycentric_coordinates = df_df_dbarycentric_coordinates_flat.data();
    std::fill(df_df_dbarycentric_coordinates, df_df_dbarycentric_coordinates + (image_height_, image_width_) * 3, 0.0f);
    ...
    for (unsigned int pixel_id = 0; pixel_id < image_height_ * image_width_;
      ...
      df_df_dbarycentric_coordinates[3 * pixel_id]      = db0_dvx * (db_dv0 * df_dvertices[v0_id] + db_dv1 * df_dvertices[v1_id] + db_dv2 * df_dvertices[v2_id]);
      df_df_dbarycentric_coordinates[3 * pixel_id]     += db0_dvy * (db_dv0 * df_dvertices[v0_id + 1] + db_dv1 * df_dvertices[v1_id + 1] + db_dv2 * df_dvertices[v2_id + 1]);
      df_df_dbarycentric_coordinates[3 * pixel_id + 1]  = db1_dvx * (db_dv0 * df_dvertices[v0_id] + db_dv1 * df_dvertices[v1_id] + db_dv2 * df_dvertices[v2_id]);
      df_df_dbarycentric_coordinates[3 * pixel_id + 1] += db1_dvy * (db_dv0 * df_dvertices[v0_id + 1] + db_dv1 * df_dvertices[v1_id + 1] + db_dv2 * df_dvertices[v2_id + 1]);
      df_df_dbarycentric_coordinates[3 * pixel_id + 2]  = db2_dvx * (db_dv0 * df_dvertices[v0_id] + db_dv1 * df_dvertices[v1_id] + db_dv2 * df_dvertices[v2_id]);
      df_df_dbarycentric_coordinates[3 * pixel_id + 2] += db2_dvy * (db_dv0 * df_dvertices[v0_id + 1] + db_dv1 * df_dvertices[v1_id + 1] + db_dv2 * df_dvertices[v2_id + 1]);
    }
  }
...
REGISTER_KERNEL_BUILDER(Name("RasterizeTrianglesGradGrad").Device(DEVICE_CPU),
                        RasterizeTrianglesGradGradOp);
